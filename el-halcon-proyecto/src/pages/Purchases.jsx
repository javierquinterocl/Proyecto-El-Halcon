...
function Purchases() {
  ...
  const apiUrl = "http://localhost:4000/api/purchases"
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/purchases/detail/${itemToDelete.invoice_purchase_id}/${itemToDelete.line_item_id}`, {
        method: "DELETE",
      })
      if (!res.ok) throw new Error(`Error ${res.status}`)
      
      // Actualizar la lista eliminando el item
      setPurchases(purchases.filter((item) => 
        !(item.invoice_purchase_id === itemToDelete.invoice_purchase_id && item.line_item_id === itemToDelete.line_item_id)
      ))
      closeDeleteModal()
    } catch (err) {
  ...
  function handleEdit(item) {
    navigate(`/dashboard/purchases/edit/${item.invoice_purchase_id}/${item.line_item_id}`)
  }

  const filteredPurchases = purchases.filter(
    (item) =>
      item.product_id?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.invoice_purchase_id?.toString().includes(searchTerm) ||
      item.line_item_id?.toLowerCase().includes(searchTerm)
  )
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h2 className="text-xl font-semibold text-gray-800">Ítems de Compra</h2>
      ...
      <div className="flex items-center gap-2 mb-4">
        <button onClick={fetchPurchases} className="flex items-center gap-1 text-sm text-gray-600 hover:text-[#FF5E0A]">
      ...
      {loading ? (
        <div className="flex-1 flex justify-center items-center">
          ...
          <p className="mt-2 text-gray-600">Cargando ítems de compra...</p>
        </div>
      ) : (
        <div className="flex-1 overflow-auto rounded-xl border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Factura Compra</th>
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {currentItems.length === 0 ? (
                ...
                currentItems.map((item) => (
                  <tr key={`${item.invoice_purchase_id}-${item.line_item_id}`} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm text-gray-700">{item.invoice_purchase_id}</td>
