...
export default function AddPurchaseInvoice() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null)
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:4000"}/api/purchaseinvoices`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      })
      if (!res.ok) throw new Error("Error al crear la factura de compra")
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-xl text-gray-700">Añadir Factura de Compra</h1>
      </div>
