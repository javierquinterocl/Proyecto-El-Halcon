...
export default function AddPurchase() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:4000/api/purchaseinvoices")
      .then(res => res.json())
      .then(setInvoices)
      .catch(err => console.error("Error al cargar facturas de compra:", err))
  }, [])

  const handleAddItem = () => {
    setPurchaseItems([...purchaseItems, { product_id: "", quantity: 1, price: 0 }])
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const itemsToSend = purchaseItems.map((item, idx) => ({
        line_item_id: String.fromCharCode(65 + idx),
        quantity: item.quantity,
        price: item.price,
        sub_total: item.quantity * item.price,
        product_id: item.product_id,
        invoice_purchase_id: parseInt(selectedInvoice),
      }))
      ...
      if (!res.ok) throw new Error("Error al guardar los ítems de compra")
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-6">
      <h2 className="text-2xl font-semibold mb-6 text-gray-800">Agregar Ítems de Compra</h2>

      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Selector de factura de compra */}
        <div>
          <label className="block text-sm font-medium mb-1 text-gray-700">
            Selecciona una Factura de Compra <span className="text-red-500">*</span>
          </label>
          <select
            required
